/*
이 스크립트는 C#의 기본적인 데이터 타입(정수, 실수, 문자열 등)과 Unity에서 자주 사용되는 타입(배열, 리스트, GameObject, Transform, Vector3, Vector2, Color, Dictionary 등)에 유용한 **확장 메서드(Extension Methods)**들을 추가하는 역할을 합니다.

간단히 말해, 이 스크립트는 다양한 타입의 객체에 대해 자주 사용되는 편리한 기능을 마치 원래 그 타입의 일부인 것처럼 사용할 수 있도록 해주는 도구 모음입니다.

주요 기능들은 다음과 같습니다.

숫자나 문자열에 대한 다양한 조작 기능 (예: 랜덤 부호 설정, 문자열에 공백 추가, 문자열을 Enum으로 변환 등)
배열과 리스트에 대한 다양한 유틸리티 기능 (예: 유니크한 랜덤 아이템 가져오기, 범위 체크, 비어 있는지 확인, 변환, 합계 계산, 필터링, 셔플, 특정 아이템 찾기 등)
GameObject와 Transform에 대한 편리한 기능 (예: 컴포넌트 가져오거나 추가하기, 스케일 뒤집기, 위치/회전/스케일 초기화 등)
Vector 및 Color 타입의 값 조작 기능 (예: 특정 축의 값 설정, 알파 값 설정, Hex 코드로 변환 등)
Dictionary에 요소를 추가하거나 값을 조정하는 기능
스크롤 UI (ScrollRect)를 특정 위치로 스냅하는 기능
요약하자면, CoreExtensions.cs는 코드 작성 시 반복되는 일반적인 작업들을 간결하게 처리할 수 있도록 미리 만들어진 유틸리티 함수들을 제공하여, 개발 효율성을 높이고 코드를 더 읽기 쉽게 만드는 역할을 합니다.
*/
