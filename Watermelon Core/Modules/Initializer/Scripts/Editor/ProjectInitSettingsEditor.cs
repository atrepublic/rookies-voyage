/*
초기화 설정 관리: ProjectInitSettings ScriptableObject 에셋의 인스펙터 창에 커스텀 UI를 제공하여 초기화 설정을 시각적으로 편집할 수 있습니다.

초기화 모듈 추가/제거: 게임 초기화 시 실행될 다양한 초기화 모듈(InitModule)들을 이 에디터 창에서 편리하게 추가하거나 제거할 수 있습니다.

필수 모듈 자동 추가: 프로젝트에 필요한 핵심 초기화 모듈이 누락되지 않도록 자동으로 감지하여 추가하는 기능을 포함합니다.

모듈 순서 변경: 추가된 초기화 모듈의 실행 순서를 드래그 앤 드롭 등으로 변경할 수 있는 UI를 제공합니다.

모듈별 커스텀 에디터 지원: 각 초기화 모듈에 대한 개별적인 커스텀 에디터 기능을 통합하여 표시합니다.

에디터 메뉴 항목 제공: Unity 메뉴에 이 설정 파일을 바로 선택할 수 있는 메뉴 항목을 추가하여 접근성을 높입니다.
*/
