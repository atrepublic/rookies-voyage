/*
📄 SlicedFilledImage.cs의 기능 요약
SlicedFilledImage는 Unity의 기본 Image 컴포넌트를 상속한 커스텀 UI 컴포넌트로, Image.Type = Sliced 모드에서 FillAmount 기능을 사용할 수 있도록 확장한 클래스야.

Unity 기본 Image 컴포넌트에서는 Image.Type이 Sliced인 경우 fillAmount 속성을 사용할 수 없어. 하지만 이 스크립트는 그 제약을 우회하여 9-slice 스프라이트에서 동적으로 채워지는 UI를 구현할 수 있도록 만들어졌어.

🧩 주요 사용 용도
체력 게이지(HP Bar) 또는 경험치 바(XP Bar) 등에서 Sliced 형태의 텍스처를 사용하면서 fillAmount 기능을 활용하고 싶을 때 사용해.

특히 UI 텍스처가 테두리 왜곡 없이 부드럽게 줄어들거나 늘어나야 할 때 유용해.
*/
